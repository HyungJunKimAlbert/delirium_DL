...
from utils.util import fix_seed, loss_plot, auc_plot, EarlyStopping, split_scaling
from models.model import CNNLSTMModel, CRNN, AttentionLSTM, TransformerLSTM
warnings.filterwarnings('ignore')

def train_one_epoch(model, dataloader, optimizer, criterion, metric, metric2, device, DATA_LENGTH):
    ...
    auc = round(metric(output_list, target_list).item(),4)
    prc = metric2(output_list, target_list.long())

    return final_loss, auc, prc

def valid_one_epoch(model, dataloader, criterion, metric, metric2, device, DATA_LENGTH):
    model.eval()
    output_list = []
    target_list = []
    running_loss = 0.0
...
# 1. Dataset
    ...
    for data_idx in [42]:
        train_data, valid_data, test_data = split_scaling(data_idx, data_path="/home/hjkim/projects/local_dev/delirium/data/dynamic/data_dynamic_2h_add_total.pkl")
        # create dataset 
        ...
        print(f"EMR: {emr_size}, VITALS: {vital_size}")
        print()
    ...
    # 2. Set options (device, Loss, etc ....)
        ...
        metric = BinaryAUROC()
        metric2 = AveragePrecision(task='binary')
        fix_seed(SEED)
        ...
        model = TransformerLSTM(emr_size, vital_size).to(device)
    ...
    # 3. Training & Validation
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            train_epoch_loss, train_auc, train_prc = train_one_epoch(model, train_dataloader, optimizer, criterion, metric, metric2, device, DATA_LENGTH)  # train
            val_epoch_loss, val_auc, val_prc = valid_one_epoch(model, valid_dataloader, criterion, metric, metric2, device, DATA_LENGTH) # valid
            ...
            print(f"Epoch [{epoch + 1}/{NUM_EPOCHS}], Train Loss: {train_epoch_loss:.4f}, Train AUC: {train_auc:.4f}, Train PRC: {train_prc:.4f}, Val Loss: {val_epoch_loss:.4f}, Val AUC: {val_auc:.4f}, Val PRC: {val_prc:.4f}")
        ...
        test_loss, test_auc, test_prc = valid_one_epoch(model, test_dataloader, criterion, metric, metric2, device, DATA_LENGTH) 
        print(f"BEST PERFORMANCE: {best_epoch}")
        print(f"Test Loss: {test_loss:.4f}, Test AUC: {test_auc:.4f}, Test PRC: {test_prc:.4f}")

        final_auc.append(test_auc)
        final_prc.append(test_prc)


    print("FINAL")
    print(f"AUC mean: {np.mean(final_auc):.4f}, Std: {np.std(final_auc):.4f}, PRC mean: {np.mean(final_prc):.4f}, Std: {np.std(final_prc):.4f}")
